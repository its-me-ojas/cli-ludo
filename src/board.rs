...
pub struct GameBoard<'a> {
    board: [[&'a str; BOARD_SIZE]; BOARD_SIZE],
}

impl<'a> GameBoard<'a> {
    // constructor
    pub fn new<'b>(players: &'b [Player; 4]) -> GameBoard<'b> {
    ...
    pub fn display_board<'b>(&self, players: &'b [Player; 4]) {
...
pub fn set_tokens_of_player<'a>(board: &mut [[&'a str; 15]; 15], players: &'a [Player; 4]) {
    for (index, player) in players.iter().enumerate() {
        ...
        for (i, j) in positions {
            for token in &player.tokens {
                let token_representation = if !token.movable {
                    &player.color[0..1]
                } else {
                    " "
                };

                board[*i][*j] = token_representation;
            }
        }
    }
}
